...
angular
  .module('emission.splash.notifscheduler', [
    'emission.services',
  ...
  .factory(
    ...
    function ($http, $window, $ionicPlatform, ClientStats, DynamicConfig, CommHelper, Logger) {
      const scheduler = {};
      let _config;
      ...
      function debugGetScheduled(prefix) {
        cordova.plugins.notification.local.getScheduled((notifs) => {
          if (!notifs?.length) return Logger.log(`${prefix}, there are no scheduled notifications`);
          ...
          Logger.log(
            `${prefix}, there are ${notifs.length} scheduled notifications at ${time} first is ${scheduler.scheduledNotifs[0].key} at ${scheduler.scheduledNotifs[0].val}`,
          );
        });
      }
      ...
      scheduler.getScheduledNotifs = function () {
        return new Promise((resolve, reject) => {
          ...
          if (isScheduling) {
            console.log(
              'requesting fetch while still actively scheduling, waiting on scheduledPromise',
            );
      ...
      const update = async () => {
        const { reminder_assignment, reminder_join_date, reminder_time_of_day } =
          await scheduler.getReminderPrefs();
