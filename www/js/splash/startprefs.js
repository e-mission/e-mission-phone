...
angular
  .module('emission.splash.startprefs', [
  ...
  .factory(
    ...
    function (
      ...
      startprefs.markConsented = function () {
        logger.log(
          'changing consent from ' +
            $rootScope.curr_consented +
            ' -> ' +
            JSON.stringify($rootScope.req_consent),
        );
        ...
        return startprefs
          ...
          .then(function (response) {
            ...
            KVStore.set(DATA_COLLECTION_CONSENTED_PROTOCOL, $rootScope.req_consent);
      ...
      startprefs.readConsentState = function () {
        ...
        return $http
          ...
          .then(function (kv_store_consent) {
            ...
            console.assert(
              angular.isDefined($rootScope.req_consent),
      ...
      startprefs.readConfig = function () {
        return DynamicConfig.loadSavedConfig().then(
          (savedConfig) => ($rootScope.app_ui_label = savedConfig),
        );
      };

      startprefs.hasConfig = function () {
        if (
          $rootScope.app_ui_label == undefined ||
      ...
      startprefs.getPendingOnboardingState = function () {
        return startprefs.readStartupState().then(function ([
          is_intro_done,
          is_consented,
          has_config,
        ]) {
          if (!has_config) {
            console.assert(
              !$rootScope.has_config,
              'in getPendingOnboardingState first check, $rootScope.has_config',
              JSON.stringify($rootScope.has_config),
            );
            return 'root.join';
          } else if (!is_intro_done) {
            console.assert(
              !$rootScope.intro_done,
              'in getPendingOnboardingState second check, $rootScope.intro_done',
              JSON.stringify($rootScope.intro_done),
            );
            return 'root.intro';
          } else {
            ...
            console.assert(
              is_intro_done,
              'in getPendingOnboardingState, local is_intro_done',
              is_intro_done,
            );
            console.assert(
              $rootScope.is_intro_done,
              'in getPendingOnboardingState, $rootScope.intro_done',
              $rootScope.intro_done,
            );
            if (is_consented) {
              return null;
      ...
      startprefs.readStartupState = function () {
        ...
        var readIntroPromise = startprefs.readIntroDone().then(startprefs.isIntroDone);
        var readConsentPromise = startprefs.readConsentState().then(startprefs.isConsented);
        var readConfigPromise = startprefs.readConfig().then(startprefs.hasConfig);
        ...
        startprefs.getConsentDocument().then(function (resultDoc) {
          if (resultDoc == null) {
            if (startprefs.isConsented()) {
              ...
              $ionicPopup.alert({
                template: 'Local consent found, native consent missing, writing consent to native',
              });
