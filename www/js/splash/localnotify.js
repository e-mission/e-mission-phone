...
angular
  .module('emission.splash.localnotify', ['emission.plugin.logger', 'ionic-toast'])
  .factory(
    'LocalNotify',
    function ($window, $ionicPlatform, $ionicPopup, $state, $rootScope, ionicToast, Logger) {
      var localNotify = {};
      ...
      localNotify.handlePrompt = function (notification, targetState, targetParams) {
        Logger.log(
          'Prompting for notification ' + notification.title + ' and text ' + notification.text,
        );
        var promptPromise = $ionicPopup.show({
        ...
        promptPromise.then(function (handle) {
          if (handle == true) {
            ...
            Logger.log(
              'Ignoring notification ' + notification.title + ' and text ' + notification.text,
            );
          }
        });
      };
      ...
      localNotify.registerRedirectHandler = function () {
        ...
        $window.cordova.plugins.notification.local.on(
          'action',
          function (notification, state, data) {
