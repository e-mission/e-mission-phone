...
angular
  .module('emission.stats.clientstats', [])
  ...
  .factory('ClientStats', function ($window) {
    ...
    clientStat.getDB = function () {
      if (
        angular.isDefined($window) &&
        angular.isDefined($window.cordova) &&
    ...
    clientStat.addReading = function (name, reading) {
      ...
      if (angular.isDefined(db)) {
        return db.putMessage(clientStat.CLIENT_TIME, clientStat.getStatsEvent(name, reading));
      }
    };
    ...
    clientStat.addEvent = function (name) {
      var db = clientStat.getDB();
      if (angular.isDefined(db)) {
        return db.putMessage(clientStat.CLIENT_NAV_EVENT, clientStat.getStatsEvent(name, null));
      }
    };
    ...
    clientStat.addError = function (name, errorStr) {
      var db = clientStat.getDB();
      if (angular.isDefined(db)) {
        return db.putMessage(clientStat.CLIENT_ERROR, clientStat.getStatsEvent(name, errorStr));
      }
    };
