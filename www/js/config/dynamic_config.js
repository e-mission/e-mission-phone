...
angular
  .module('emission.config.dynamic', ['emission.plugin.logger', 'emission.plugin.kvstore'])
  .factory(
    'DynamicConfig',
    function ($http, $ionicPlatform, $window, $state, $rootScope, $timeout, KVStore, Logger) {
      ...
      dc.configReady = function () {
        if (dc.isConfigReady) {
          ...
          return new Promise(function (resolve, reject) {
            Logger.log(
              'Registering for UI_CONFIG_READY notification in dynamic_config inside the promise',
            );
            $rootScope.$on(dc.UI_CONFIG_READY, (event, newConfig) => {
              Logger.log(
                'Received UI_CONFIG_READY notification in dynamic_config, resolving promise',
              );
              resolve(newConfig);
            });
          });
      ...
      const readConfigFromServer = async (label) => {
        ...
        Logger.log(
          'Successfully found config, result is ' + JSON.stringify(config).substring(0, 10),
        );
      ...
      dc.loadSavedConfig = function () {
        ...
        return Promise.all([rwDocRead, kvDocRead])
          .then(([rwConfig, kvStoreConfig]) => {
            ...
            Logger.log(
              'DYNAMIC CONFIG: kvStoreConfig key length = ' +
                Object.keys(kvStoreConfig || {}).length +
                ' rwConfig key length = ' +
                Object.keys(rwConfig || {}).length +
                ' using kvStoreConfig? ' +
                (kvStoreConfig ? true : false),
            );
            if (!kvStoreConfig && rwConfig) {
              // Backwards compat, can remove at the end of 2023
              Logger.log(
                'DYNAMIC CONFIG: rwConfig found, kvStoreConfig not found, setting to fix backwards compat',
              );
              KVStore.set(CONFIG_PHONE_UI_KVSTORE, rwConfig);
            }
            if (
              Object.keys(kvStoreConfig || {}).length > 0 &&
      ...
      dc.resetConfigAndRefresh = function () {
        const resetNativePromise = $window.cordova.plugins.BEMUserCache.putRWDocument(
          CONFIG_PHONE_UI,
          {},
        );
        const resetKVStorePromise = KVStore.set(CONFIG_PHONE_UI_KVSTORE, {});
        Promise.all([resetNativePromise, resetKVStorePromise]).then($window.location.reload(true));
      };
      ...
      var loadNewConfig = function (newStudyLabel, thenGoToIntro, existingVersion = null) {
        return readConfigFromServer(newStudyLabel)
          .then((downloadedConfig) => {
            ...
            const logSuccess = (storeResults) =>
              Logger.log(
                'UI_CONFIG: Stored dynamic config successfully, result = ' +
                  JSON.stringify(storeResults),
              );
        ...
        if (config.opcode) {
          ...
          if (config.opcode.subgroups) {
            if (config.opcode.subgroups.indexOf(tokenParts[2]) == -1) {
              ...
              console.log(
                'subgroup ' + tokenParts[2] + ' found in list ' + config.opcode.subgroups,
              );
              return tokenParts[2];
            }
          } else {
      ...
      dc.initByUser = function (urlComponents) {
        ...
        try {
          ...
          return (
            loadNewConfig(newStudyLabel, true)
              // on successful download, cache the token in the rootScope
              .then((wasUpdated) => {
                $rootScope.scannedToken = dc.scannedToken;
              })
              .catch((fetchErr) => {
        ...
        dc.loadSavedConfig()
          .then((existingConfig) => {
            ...
            if (existingConfig.autoRefresh) {
              ...
              Logger.log(
                'UI_CONFIG: autoRefresh is false, not checking for updates. Using existing config',
              );
