...
angular
  .module('emission.config.imperial', ['emission.plugin.logger'])
  .factory('ImperialConfig', function ($rootScope, DynamicConfig, Logger, $ionicPlatform) {
    ...
    ic.getFormattedDistanceInMiles = function (dist_in_meters) {
      return Number.parseFloat(
        (KM_TO_MILES * ic.getFormattedDistanceInKm(dist_in_meters)).toFixed(1),
      );
    };
    ...
    ic.init = function () {
      ic.getFormattedDistance = ic.useImperial
        ? ic.getFormattedDistanceInMiles
    ...
    $ionicPlatform.ready().then(function () {
      ...
      DynamicConfig.configReady()
        .then((newConfig) => {
          Logger.log(
            'UI_CONFIG: Resolved configReady promise in imperial.js, setting display_config ' +
              JSON.stringify(newConfig.display_config),
          );
