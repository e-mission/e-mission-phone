angular.module("emission.main.metrics.factory", [
    ...
    cc.getCorrectedMet = function(met, gender, age, height, heightUnit, weight, weightUnit) {
        height = heightUnit == 0? fttocm(height) : height;
        weight = weightUnit == 0? lbtokg(weight) : weight;
        if (gender == 1) { //male
            const met = met*3.5/((66.4730+5.0033*height+13.7516*weight-6.7550*age)/ 1440 / 5 / weight * 1000);
            return met;
        } else if (gender == 0) { //female
            const met = met*3.5/((655.0955+1.8496*height+9.5634*weight-4.6756*age)/ 1440 / 5 / weight * 1000);
            return met;
        }
    };
