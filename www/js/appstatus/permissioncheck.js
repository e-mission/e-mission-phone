    ...
    function ($scope, $element, $attrs, $ionicPlatform, $ionicPopup, $window) {
      ...
      $scope.setupAndroidFitnessChecks = function (platform, version) {
        ...
        let fixPerms = function () {
          ...
          return checkOrFix(
            ...
            $scope.recomputeFitnessStatus,
            true,
          ).then((error) => (fitnessPermissionsCheck.desc = error));
        };
        let checkPerms = function () {
          console.log('fix and refresh fitness permissions');
          return checkOrFix(
      ...
      $scope.setupAndroidNotificationChecks = function () {
        let fixPerms = function () {
          ...
          return checkOrFix(
            ...
            $scope.recomputeNotificationStatus,
            true,
          );
        };
        let checkPerms = function () {
      ...
      $scope.setupPermissionText = function () {
        if ($scope.platform.toLowerCase() == 'ios') {
          if ($scope.osver < 13) {
            $scope.locationPermExplanation = i18next.t(
