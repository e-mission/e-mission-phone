...
angular
  .module('emission.survey.enketo.answer', ['ionic', 'emission.config.dynamic'])
  .factory('EnketoSurveyAnswer', function ($http, DynamicConfig) {
    ...
    function filterByNameAndVersion(name, answers) {
      return _lazyLoadConfig().then((config) =>
        answers.filter(
          (answer) =>
            answer.data.name === name && answer.data.version >= config[name].compatibleWith,
        ),
      );
    }
    ...
    function resolveLabel(name, xmlDoc) {
      ...
      if (LABEL_FUNCTIONS[name]) return LABEL_FUNCTIONS[name](xmlDoc);
