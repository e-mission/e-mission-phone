...
angular
  .module('emission.survey.enketo.service', [
    'ionic',
    'emission.services',
  ...
  .factory(
    'EnketoSurvey',
    function ($window, $http, UnifiedDataLoader, InputMatcher, EnketoSurveyAnswer, DynamicConfig) {
      /**
      ...
      function _saveData() {
        ...
        return EnketoSurveyAnswer.resolveLabel(_state.name, xmlDoc)
          .then((rsLabel) => {
            const data = {
              label: rsLabel,
              name: _state.name,
            ...
            if (_state.opts.timelineEntry) {
              let timestamps = EnketoSurveyAnswer.resolveTimestamps(
                xmlDoc,
                _state.opts.timelineEntry,
              );
