    ...
    function (InputMatcher, EnketoSurveyAnswer, Logger, $timeout) {
      ...
      enbs.extractResult = function (results) {
        const resultsPromises = [
          EnketoSurveyAnswer.filterByNameAndVersion(enbs.timelineEntrySurveyName, results),
        ];
        if (enbs.timelineEntrySurveyName != enbs.placeSurveyName) {
          resultsPromises.push(
            EnketoSurveyAnswer.filterByNameAndVersion(enbs.placeSurveyName, results),
          );
        }
      ...
      enbs.populateInputsAndInferences = function (timelineEntry, manualResultMap) {
        console.log(
          'ENKETO: populating timelineEntry,',
          timelineEntry,
          ' with result map',
          manualResultMap,
        );
      ...
      enbs.populateManualInputs = function (timelineEntry, inputType, inputList) {
        ...
        Logger.log(
          'After combining unprocessed (' +
            ...
            ') with server (' +
            timelineEntry.additions.length +
            ') for a combined (' +
            combinedPotentialAdditionList.length +
            '), deduped entries are (' +
            dedupedList.length +
            ')',
        );
