...
angular
  .module('emission.survey.enketo.demographics', [
    ...
    function (
      ...
      $scope.openPopover = function ($event) {
        return EnketoDemographicsService.loadPriorDemographicSurvey().then((lastSurvey) => {
          return EnketoSurveyLaunch.launch($scope, 'UserProfileSurvey', {
            prefilledSurveyResponse: lastSurvey?.data?.xmlResponse,
            showBackButton: true,
            showFormFooterJumpNav: true,
          }).then((result) => {
  ...
  .controller(
    ...
    function (
      ...
      var validateAndSave = function () {
        return EnketoSurvey.validateAndSave().then((result) => {
          if (!result) {
      ...
      $scope.setEditSurveyAnswer = function (newVal) {
        ...
        $timeout(() => {
          if (newVal) {
             ...
             */
            return EnketoSurveyLaunch.initSurvey('UserProfileSurvey', {
              prefilledSurveyResponse: $scope.existingSurvey?.data?.xmlResponse,
              showBackButton: true,
              showFormFooterJumpNav: true,
            })
              .then((result) => {
      ...
      $scope.initForm = function () {
        $scope.loading = true;
        return EnketoDemographicsService.loadPriorDemographicSurvey()
          .then((lastSurvey) => {
            $scope.$apply(() => {
              $scope.loading = false;
              $scope.existingSurvey = lastSurvey;
            });
  ...
  .factory('EnketoDemographicsService', function (UnifiedDataLoader, $window, $ionicLoading) {
    ...
    eds.loadPriorDemographicSurvey = function () {
      ...
      return UnifiedDataLoader.getUnifiedMessagesForInterval(eds.key, tq).then((answers) =>
