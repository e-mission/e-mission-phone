...
angular
  .module('emission.survey.multilabel.buttons', [
  ...
  .factory(
    'MultiLabelService',
    function ($rootScope, InputMatcher, $timeout, $ionicPlatform, DynamicConfig, Logger) {
      var mls = {};
      ...
      $ionicPlatform.ready().then(function () {
        ...
        DynamicConfig.configReady()
          .then((newConfig) => {
            mls.init(newConfig);
          })
          .catch((err) =>
            Logger.displayError('Error while handling config in MultiLabelService', err),
          );
      });
      ...
      mls.populateInputsAndInferences = function (trip, manualResultMap) {
        if (angular.isDefined(trip)) {
          ...
          getLabelInputs().forEach(function (item, index) {
            mls.populateManualInputs(trip, trip.nextTrip, item, manualResultMap[item]);
          });
          trip.finalInference = {};
      ...
      mls.populateInput = function (tripField, inputType, userInputLabel) {
        if (angular.isDefined(userInputLabel)) {
          console.log(
            'populateInput: looking in map of ' +
              inputType +
              ' for userInputLabel' +
              userInputLabel,
          );
          ...
          console.log(
            'Mapped label ' + userInputLabel + ' to entry ' + JSON.stringify(userInputEntry),
          );
      ...
      mls.inferFinalLabels = function (trip) {
        ...
        const totalCertainty = labelsList
          .map((item) => item.p)
          .reduce((item, rest) => item + rest, 0);
        ...
        if (labelsList.length == 0) {
          for (const inputType of getLabelInputs())
            mls.populateInput(trip.finalInference, inputType, undefined);
        } else {
      ...
      mls.expandInputsIfNecessary = function (trip) {
        ...
        if (mls.ui_config.intro.mode_studied) {
          if (inputValue == mls.ui_config.intro.mode_studied) {
            Logger.log(
              'Found ' +
                mls.ui_config.intro.mode_studied +
                ' mode in a program, displaying full details',
            );
            ...
            Logger.log(
              'Found non ' +
                mls.ui_config.intro.mode_studied +
                ' mode in a program, displaying base details',
            );
