...
angular
  .module('emission.survey.multilabel.infscrollfilters', ['emission.plugin.logger'])
  .factory('MultiLabelInfScrollFilters', function (Logger) {
    var sf = {};
    var unlabeledCheck = function (t) {
      return t.INPUTS.map((inputType, index) => !angular.isDefined(t.userInput[inputType])).reduce(
        (acc, val) => acc || val,
        false,
      );
    };
    ...
    var invalidCheck = function (t) {
      const retVal =
        angular.isDefined(t.userInput['MODE']) &&
