...
angular
  .module('emission.survey.external.launch', [
    'emission.services',
  ...
  .factory(
    'SurveyLaunch',
    function ($http, $window, $ionicPopup, $rootScope, ClientStats, CommHelper, Logger) {
      var surveylaunch = {};
      var replace_uuid = function (iab, elementSelector, scriptFile) {
        return Promise.all([$http.get(scriptFile), CommHelper.getUser()]).then(function ([
          scriptText,
          userProfile,
        ]) {
          ...
          Logger.log(
            'inserting user id into survey. userId = ' +
              uuid +
              ' element selector = ' +
              elementSelector,
          );
          var codeTemplate = scriptText.data;
          var codeString = codeTemplate
            .replace('SCRIPT_REPLACE_VALUE', uuid)
      ...
      var replace_time = function (iab, tsElementId, fmtTimeElementId, ts, label) {
        ...
        return Promise.all([$http.get('js/survey/time_insert.js')]).then(function ([scriptText]) {
          // alert("finished loading script");
          Logger.log(scriptText.data);
          // I tried to use http://stackoverflow.com/posts/23387583/revisions
          // for the idea on how to invoke the function in the script
          ...
          // script file directly and insert the userId.
          Logger.log(
            'inserting ts into survey. ts = ' +
              ts +
              ' element id = ' +
              tsElementId +
              ' fmtTime' +
              moment.unix(ts).format() +
              ' element id = ' +
              fmtTimeElementId,
          );
          var codeTemplate = scriptText.data;
          var tsCodeString = codeTemplate
            .replace('SCRIPT_REPLACE_VALUE', ts)
          ...
          var fmtTimeCodeString = codeTemplate
            .replace('SCRIPT_REPLACE_VALUE', moment.unix(ts).format())
    ...
    */
      ...
      surveylaunch.startSurveyForCompletedTrip = function (
        url,
        uuidElementId,
        startTsElementId,
        endTsElementId,
        ...
        iab.addEventListener('loadstop', function (event) {
          ...
          // success
          Promise.all([
            replace_uuid(iab, uuidElementId, 'js/survey/uuid_insert.js'),
            ...
            // catch for all promises
            $ionicPopup
              .alert({
                template:
                  'Relaunching survey - while replacing uuid, got error ' + JSON.stringify(error),
              })
              .then(function () {
                surveylaunch.startSurvey(
                  url,
                  uuidElementId,
                  startTsElementId,
                  endTsElementId,
      ...
      var startSurveyCommon = function (url, elementSelector, elementSelScriptFile) {
        ...
        iab.addEventListener('loadstop', function (event) {
          console.log('successfully opened page with result ' + JSON.stringify(event));
          // success
          replace_uuid(iab, elementSelector, elementSelScriptFile).catch(function (error) {
            iab.close();
            $ionicPopup
              .alert({ template: 'Relaunching survey - while replacing uuid, got error ' + error })
              .then(function () {
      ...
      surveylaunch.startSurveyPrefilled = function (url, opts = {}) {
        ...
        return CommHelper.getUser().then(function (userProfile) {
          ...
          Logger.log('inserting user id into survey. userId = ' + uuid + ' base url = ' + url);
      ...
      surveylaunch.init = function () {
        $rootScope.$on('cloud:push:notification', function (event, data) {
          ClientStats.addEvent(ClientStats.getStatKeys().NOTIFICATION_OPEN).then(function () {
            console.log(
              'Added ' +
                ClientStats.getStatKeys().NOTIFICATION_OPEN +
                ' event. Data = ' +
                JSON.stringify(data),
            );
          });
          ...
          if (
            ...
            if (angular.isDefined(survey_spec) && angular.isDefined(survey_spec.url)) {
              if (angular.isDefined(survey_spec.uuidElementId)) {
                ...
                $ionicPopup.alert(
                  'survey was not specified correctly. spec is ' + JSON.stringify(survey_spec),
                );
              }
            } else {
              $ionicPopup.alert(
