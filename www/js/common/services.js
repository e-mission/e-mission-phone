angular.module("emission.main.common.services", [
    ...
    var binEntries = function(toBinArray, binFn) {
        ...
        toBinArray.forEach(function(entry, _index, _array) {
            var key = binFn(entry);
    ...
    commonGraph.getCommonTripForStartEnd = function(commonStartPlaceId, commonEndPlaceId) {
        var retMap = new Map();
        commonGraph.data.graph.common_trips.forEach(function(cTrip, _index, _array) {
    ...
    commonGraph.getDisplayName = function(mode, obj) {
        ...
        switch (mode) {
        case "place":
            url = "http://nominatim.openstreetmap.org/reverse?format=json&lat=" + obj.geometry.coordinates[1]
          + "&lon=" + obj.geometry.coordinates[0];
            ...
            break;
        case "cplace":
            url = "http://nominatim.openstreetmap.org/reverse?format=json&lat=" + obj.location.coordinates[1]
        + "&lon=" + obj.location.coordinates[0];
            ...
            break;
        case "ctrip":
            url = "http://nominatim.openstreetmap.org/reverse?format=json&lat=" + obj.start_loc.coordinates[1]
          + "&lon=" + obj.start_loc.coordinates[0];
            ...
            url = "http://nominatim.openstreetmap.org/reverse?format=json&lat=" + obj.end_loc.coordinates[1]
    ...
    var getKeyWithMaxVal = function(kv_map) {
        ...
        kv_array.forEach(function(entry, _index, _array) {
    ...
    var postProcessData = function() {
        ...
        commonGraph.data.graph.common_trips.forEach(function(cTrip, _index, _array) {
            ...
            cTrip.trips.forEach(function(tripId,_index,_array) {
        ...
        commonGraph.data.graph.common_places.forEach(function(cPlace, _index, _array) {
            ...
            cPlace.places.forEach(function(placeId,_index,_array) {
                commonGraph.data.place2CommonMap[placeId.$oid] = cPlace;
            });
        });
        commonGraph.data.graph.common_places.forEach(function(cPlace, _index, _array) {
            cPlace.succ_places = cPlace.successors.map(function(succId, _index, _array) {
