...
angular.module('emission.main.diary.list', [
  ...
  controller('DiaryListCtrl',
    function ($window, $scope, $rootScope, $ionicPlatform, $state,
      ...
      */
      ...
      $scope.listLocationClass = function () {
        return ($scope.dark_theme)
          ? 'item item-icon-left list-location-dark'
          : 'item item-icon-left list-location';
      };
      ...
      $scope.listCardClass = function (tripgj) {
        var background = DiaryHelper.getTripBackground($scope.dark_theme,
          tripgj);
      ...
      $scope.listColLeftClass = function (margin) {
        if (margin == 0) {
          return ($scope.dark_theme)
            ? 'col-50 list-col-left-dark'
            : 'col-50 list-col-left';
        } else {
          return ($scope.dark_theme)
            ? 'col-50 list-col-left-margin-dark'
            : 'col-50 list-col-left-margin';
        }
      };
      $scope.listColRightClass = function () {
        return ($scope.dark_theme)
          ? 'col-50 list-col-right-dark'
          : 'col-50 list-col-right';
      };
      $scope.differentCommon = function (tripgj) {
        return ($scope.isCommon(tripgj.id))
          ? ((DiaryHelper.getEarlierOrLater(tripgj.data.properties.start_ts,
            tripgj.data.id) == '') ? false : true)
          : false;
      };
      $scope.stopTimeTagClass = function (tripgj) {
        return ($scope.differentCommon(tripgj))
          ? 'stop-time-tag-lower'
          : 'stop-time-tag';
      };
      $scope.setCurrDay = function (val) {
        if (typeof(val) === 'undefined') {
          ...
          window.Logger.log(window.Logger.LEVEL_INFO, 'Selected date is :' +
            val);
      ...
      $scope.$on(CommonGraph.UPDATE_DONE, function (event, args) {
        console.log('Got common graph update done event with args ' +
          JSON.stringify(args));
        $scope.$apply(function () {
          ...
          if (!angular.isUndefined($scope.data) &&
            !angular.isUndefined($scope.data.currDayTripWrappers)) {
            $scope.data.currDayTripWrappers.forEach(
              function (tripWrapper, index, array) {
        ...
        var buttons = [
          {
            ...
            onTap: function (e) { $state.go('root.main.recent.log'); },
          },
          {
            text: 'Force',
            type: 'button-balanced',
            onTap: function (e) { $state.go('root.main.control'); },
          },
          {
            text: 'OK',
            type: 'button-balanced',
      ...
      */
      ...
      $scope.prevDay = function () {
        console.log('Called prevDay when currDay = ' +
          Timeline.data.currDay.format('YYYY-MM-DD'));
