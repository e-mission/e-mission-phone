angular.module("emission.main.diary.timeline-helper.service", [
    ...
    */
    // var readAndUpdateFromFile = function (day, _foundFn, _notFoundFn) {
    ...
    */
    ...
    var place2Geojson = function (_trip, locationPoint, propertyFiller) {
        var place_gj = {
    ...
    var points2Geojson = function (_trip, locationPoints) {
        ...
        var distances = [0];
        for (let i = 0; i < leafletLatLng.length - 1; i++) {
    ...
    timeline.readUnprocessedTrips = function (day, tripListForDay) {
      ...
      */
        ...
        var last_processed_ts;
        if (tripListForDay.length == 0) {
            last_processed_ts = moment(day).startOf("day").unix();
        } else {
            last_processed_ts = tripListForDay[tripListForDay.length - 1].properties.end_ts;
        }
    ...
    */
    var processTripsForDay = function (day, tripListForDay) {
        ...
        tripListForDay.forEach(function (item, index, _array) {
        ...
        timeline.data.currDayTrips.forEach(function (trip, _index, _array) {
    ...
    timeline.setTripWrappers = function (tripWrapperList) {
        ...
        timeline.data.currDayTripWrappers.forEach(function (tripw, _index, _array) {
    ...
    var generateDaySummary = function () {
        ...
        timeline.data.currDayTrips.forEach(function (trip, _index, _array) {
            ...
            var movingTime = 0;
            var stoppedTime = 0;
            trip.stops.forEach(function (stop, _index, _array) {
                stoppedTime = stoppedTime + stop.properties.duration;
            });
            trip.sections.forEach(function (section, _index, _array) {
    ...
    var getTripComponents = function (trip) {
        ...
        var stopList = [];
        var sectionList = [];
        trip.features.forEach(function (feature, _index, _array) {
            ...
            case "FeatureCollection":
                feature.features.forEach(function (item, _index, _array) {
