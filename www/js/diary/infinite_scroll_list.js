...
.controller("InfiniteDiaryListCtrl", function($window, $scope, $rootScope, $ionicPlatform, $state,
  ...
  /**
   * Check whether we should be using the new label UI or not
   */
  $scope.updateNewLabel = function() {
    const url = $rootScope.connectUrl;
    Logger.log(url == undefined ? "URL UNDEFINED" : "URL DEFINED");
    if (url == undefined) return;  //Hopefully this doesn't happen in practice...
    $scope.newLabel = url.includes("stage");
    $scope.newLabel = false;  //For testing
