angular.module("emission.main.diary.current", [
    "ui-leaflet",
    "ngCordova", 
    "emission.services", 
    "ionic",
...
]).controller("CurrMapCtrl", function(
    $scope, Config, $state, leafletData, 
    Logger, PostTripManualMarker, CommHelper,
    $http, KVStore, $ionicPlatform
) {
    ...
    const MANUAL_INCIDENT = "manual/incident";
    const BACKGROUND_LOCATION = "background/location";
    const INCIDENT_CONFIG = "incident_config";
    $scope.mapCtrl = {}; 
    ...
    $scope.$watch("verticalSlider.value", function(newVal, _oldVal){
    ...
    var getSpeed = function(curr_lglat, last_lglat, curr_ts, last_ts) {
        ...
        const time = moment(curr_ts).diff(moment(last_ts));
    ...
    var refreshTrip = function() {
        db().getAllSensorData(BACKGROUND_LOCATION, true).then(function(result) {
            $scope.$apply(function() {
                ...
                var coordinates = result.map(function(locWrapper, _index, _locList) {
    ...
    $scope.$on("leafletDirectiveMap.current.resize", function(_event, data) {
        $ionicPlatform.ready().then(function() {
    ...
    $scope.showIncidentSheet = function() {
        db().getAllSensorData(BACKGROUND_LOCATION, true).then(function(result) {
            const both = [result[0].data.latitude, result[0].data.longitude];
    ...
    $scope.$watch("features", function(newVal, _oldVal){
    ...
    $ionicPlatform.on("resume", function(_event) {
        Logger.log("resumed current screen, starting incident refresh");
        refreshTripLoop();
        getIncidentsLoop();
    });

    $ionicPlatform.on("pause", function(_event) {
