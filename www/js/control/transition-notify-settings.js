angular.module("emission.main.control.tnotify", [
...
]).factory("ControlTransitionNotifyHelper", function(
    ...
    ctnh.config2notifyList = function(configWithMetadata) {
        ...
        var enabledList = configList.map(function(config, _i) {
    ...
    ctnh.saveAndReload = function() {
        ...
        Promise.all(promiseList).then(function(_resultList) {
    ...
    ctnh.toggleLowAccuracy = function() {
        ...
        if (ctnh.isMediumAccuracy()) {
            if (ionic.Platform.isIOS()) {
                ...
                ctnh.accuracyOptions["PRIORITY_HIGH_ACCURACY"];
            }
        } else {
            if (ionic.Platform.isIOS()) {
