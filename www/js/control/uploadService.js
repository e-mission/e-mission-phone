...
angular
  .module('emission.services.upload', ['emission.plugin.logger'])
  ...
  .service('UploadHelper', function ($window, $http, $rootScope, $ionicPopup, Logger) {
    const getUploadConfig = function () {
      return new Promise(function (resolve, reject) {
        ...
        var url = [];
        $http
          .get('json/uploadConfig.json')
          .then(function (uploadConfig) {
            Logger.log(
              Logger.LEVEL_DEBUG,
              'uploadConfigString = ' + JSON.stringify(uploadConfig.data),
            );
          ...
          .catch(function (err) {
            $http
              .get('json/uploadConfig.json.sample')
              .then(function (uploadConfig) {
                Logger.log(
                  Logger.LEVEL_DEBUG,
                  'default uploadConfigString = ' + JSON.stringify(uploadConfig.data),
                );
                url.push(uploadConfig.data.url);
                resolve(url);
              })
    ...
    this.uploadFile = function (database) {
      getUploadConfig()
        .then((uploadConfig) => {
          ...
          newScope.data = {};
          newScope.fromDirText = i18next.t('upload-service.upload-from-dir', {
            parentDir: parentDir,
          });
          newScope.toServerText = i18next.t('upload-service.upload-to-server', {
          ...
          Promise.all(readFileAndInfo)
            .then(([binString, reason]) => {
              if (!didCancel) {
                ...
                uploadConfig.forEach((url) => {
                  const progressPopup = $ionicPopup.show({
                    title: i18next.t('upload-service.upload-database', { db: database }),
                    template:
                      i18next.t('upload-service.upload-progress', {
                        filesizemb: binString.byteLength / (1000 * 1000),
                        serverURL: uploadConfig,
                      }) + '<center><ion-spinner></ion-spinner></center>',
                    scope: progressScope,
                    buttons: [{ text: '<b>Cancel</b>', type: 'button-cancel' }],
                  });
                  sendToServer(url, binString, params)
                    .then((response) => {
