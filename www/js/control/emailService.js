...
angular
  .module('emission.services.email', ['emission.plugin.logger'])
  ...
  .service('EmailHelper', function ($window, $http, Logger) {
    const getEmailConfig = function () {
      return new Promise(function (resolve, reject) {
        ...
        var address = [];
        $http
          .get('json/emailConfig.json')
          .then(function (emailConfig) {
            window.Logger.log(
              window.Logger.LEVEL_DEBUG,
              'emailConfigString = ' + JSON.stringify(emailConfig.data),
            );
          ...
          .catch(function (err) {
            $http
              .get('json/emailConfig.json.sample')
              .then(function (emailConfig) {
                window.Logger.log(
                  window.Logger.LEVEL_DEBUG,
                  'default emailConfigString = ' + JSON.stringify(emailConfig.data),
                );
                address.push(emailConfig.data.address);
                resolve(address);
              })
              .catch(function (err) {
                window.Logger.log(
                  window.Logger.LEVEL_ERROR,
                  'Error while reading default email config' + err,
                );
                reject(err);
              });
          });
