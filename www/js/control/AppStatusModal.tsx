...
const AppStatusModal = ({ permitVis, setPermitVis }) => {
  ...
  function createChecklist() {
    console.debug(
      'setting up checks, platform is ' + platform.current + 'and osver is ' + osver.current,
    );
  ...
  useEffect(() => {
    ...
    if (!currentlyOpen && overallStatus == false && appConfig && haveSetText) {
      //trying to block early cases from throwing modal
    ...
    <>
      <Modal visible={permitVis} onDismiss={() => setPermitVis(false)} transparent={true}>
        <Dialog
          ...
          <Dialog.Actions>
            <Button onPress={() => refreshAllChecks()}>{t('intro.appstatus.refresh')}</Button>
            <Button onPress={() => setPermitVis(false)} disabled={!overallStatus}>
