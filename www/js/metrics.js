angular.module("emission.main.metrics",[
    "nvd3",
    "emission.services",
    "ionic-datepicker",
    ...
    var setMetrics = function(mode, callback) {
        ...
        if (mode === "local_date") { // local_date filter
            tempFrom = $scope.selectCtrl.fromDateLocalDate;
            tempFrom.weekday = $scope.selectCtrl.fromDateWeekdayValue;
            tempTo = $scope.selectCtrl.toDateLocalDate;
            ...
            if(lastTwoWeeksQuery) {
                tempFrom = moment2Timestamp(moment().utc().startOf("day").subtract(14, "days"));
                tempTo = moment2Timestamp(moment().utc().startOf("day").subtract(1, "days"));
                lastTwoWeeksQuery = false; // Only get last week's data once
            } else {
                tempFrom = moment2Timestamp($scope.selectCtrl.fromDateTimestamp);
                tempTo = moment2Timestamp($scope.selectCtrl.toDateTimestamp);
    ...
    $scope.fillUserValues = function(user_metrics_arr) {
        let seventhDayAgo = moment().utc().startOf("day").subtract(7, "days");
        let twoWeeksAgoDuration = [];
        let twoWeeksAgoMedianSpeed = [];
        let twoWeeksAgoDistance = [];
        let userDuration = [];
        let userMedianSpeed = [];
        let userCount = [];
        let userDistance = [];
        ...
        if(first){
            for(let i in user_metrics_arr[0]) {
            ...
            userDuration = user_metrics_arr[0];
            userMedianSpeed = user_metrics_arr[1];
            userCount = user_metrics_arr[2];
            userDistance = user_metrics_arr[3];
        }
    ...
    $scope.fillAggregateValues = function(agg_metrics_arr) {
        let aggDuration, aggMedianSpeed, aggCount, aggDistance;
        if (first) {
            aggDuration = agg_metrics_arr[0].slice(0, 7);
            aggMedianSpeed = agg_metrics_arr[1].slice(0, 7);
            aggCount = agg_metrics_arr[2].slice(0, 7);
            aggDistance = agg_metrics_arr[3].slice(0, 7);
        } else {
            aggDuration = agg_metrics_arr[0];
            aggMedianSpeed = agg_metrics_arr[1];
            aggCount = agg_metrics_arr[2];
            aggDistance = agg_metrics_arr[3];
        }
    ...
    $scope.fillCalorieCardUserVals = function(
        userDuration, userMedianSpeed,
        twoWeeksAgoDuration, twoWeeksAgoMedianSpeed
    ) {
        let durationData;
        let speedData;
        if (userDuration) {
            durationData = getSummaryDataRaw(userDuration, "duration");
        }
        if (userMedianSpeed) {
            speedData = getSummaryDataRaw(userMedianSpeed, "median_speed");
        }
        for (let i in durationData) {
            let met = $scope.getCorrectedMetFromUserData(durationData[i], speedData[i]);
        ...
        if(first){
            if (twoWeeksAgoDuration) {
                durationData = getSummaryDataRaw(twoWeeksAgoDuration, "duration");
            }
            if (twoWeeksAgoMedianSpeed) {
                speedData = getSummaryDataRaw(twoWeeksAgoMedianSpeed, "median_speed");
            }
            for (let i in durationData) {
                let met = $scope.getCorrectedMetFromUserData(durationData[i], speedData[i]);
                twoWeeksAgoCalories +=
    ...
    $scope.fillFootprintCardUserVals = function(userDistance, twoWeeksAgoDistance) {
        let userCarbonData;
        if (userDistance) {
            userCarbonData = getSummaryDataRaw(userDistance, "distance");
        ...
        if (first) {
            if (twoWeeksAgoDistance) {
                userCarbonData = getSummaryDataRaw(twoWeeksAgoDistance, "distance");
                for (let i in userCarbonData) {
                    if (userCarbonData[i].key === "IN_VEHICLE") {
        ...
        var calculation = (
            (
                (lastWeekCarbonInt[0] + lastWeekCarbonInt[1]) / 2
            ) / (
    ...
    var getAvgDataFromMetrics = function(metrics) {
        ...
        for (var mode in mode_bins) {
            rtn.push({key: mode, values: mode_bins[mode]});
        }
        return rtn;
    };
