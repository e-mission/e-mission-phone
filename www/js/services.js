...
angular
  .module('emission.services', ['emission.plugin.logger', 'emission.plugin.kvstore'])
  ...
  .service('CommHelper', function ($rootScope) {
    ...
    this.getRawEntriesForLocalDate = function (
      key_list,
      start_ts,
      end_ts,
    ...
    this.getAggregateData = function (path, data) {
      return new Promise(function (resolve, reject) {
        ...
        if ($rootScope.aggregateAuth === 'no_auth') {
          console.log(
            'getting aggregate data without user authentication from ' +
              full_url +
              ' with arguments ' +
              JSON.stringify(data),
          );
          ...
          console.log(
            'getting aggregate data with user authentication from ' +
              full_url +
              ' with arguments ' +
              JSON.stringify(data),
          );
  ...
  .service('UnifiedDataLoader', function ($window, CommHelper, Logger) {
    ...
    this.getUnifiedSensorDataForInterval = function (key, tq) {
      ...
      var remotePromise = CommHelper.getRawEntries([key], tq.startTs, tq.endTs).then(
  ...
  .service('ControlHelper', function ($window, $ionicPopup, CommHelper, Logger) {
    ...
    this.getMyData = function (startTs) {
      ...
      var dumpFile = startMoment.format(fmt) + '.' + endMoment.format(fmt) + '.timeline';
        ...
        return new Promise(function (resolve, reject) {
          ...
          window.requestFileSystem(window.LocalFileSystem.TEMPORARY, 0, function (fs) {
            ...
            fs.root.getFile(dumpFile, { create: true, exclusive: false }, function (fileEntry) {
              console.log(
                'fileEntry ' + fileEntry.nativeURL + ' is file?' + fileEntry.isFile.toString(),
              );
          ...
          window.requestFileSystem(window.LocalFileSystem.TEMPORARY, 0, function (fs) {
            ...
            fs.root.getFile(dumpFile, null, function (fileEntry) {
              ...
              fileEntry.file(
                  ...
                  reader.onloadend = function () {
                    ...
                    var email = {
                      ...
                      subject: i18next.t('email-service.email-data.subject-data-dump-from-to', {
                        start: startMoment.format(fmt),
                        end: endMoment.format(fmt),
                      }),
                      body: i18next.t(
                ...
                function (error) {
                  $ionicPopup.alert({
                    title: 'Error while downloading JSON dump',
                    template: error,
                  });
                  reject(error);
                },
