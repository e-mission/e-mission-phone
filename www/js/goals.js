angular.module("emission.main.goals",["emission.services", "emission.plugin.logger",
    ...
    CommHelper, $state, $ionicLoading, $ionicPlatform, $scope, $rootScope, $ionicModal, nzTour,
    $ionicPopup, $timeout, KVStore, ReferralHandler, ReferHelper, Logger, $cordovaInAppBrowser,
    SurveyLaunch
) {
    $scope.goals = [];
    $scope.goal = {};
    ...
    $rootScope.$on("RELOAD_GOAL_PAGE_FOR_REFERRAL", function(_event) {
    ...
    $scope.toBrowser = function() {     
        ...
        $rootScope.$on("$cordovaInAppBrowser:loadstop", function(_e, event){     
    ...
    $scope.deleteGoal = function(taskId,  goal) {
        ...
        CommHelper.habiticaProxy(callOpts).then(function(_response){
